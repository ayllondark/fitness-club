import React from 'react';
import { Header } from '../Header/Header';
import './Hero.css';
import hero_image from '../../assets/hero_image.png';
import hero_image_back from '../../assets/hero_image_back.png';
import Heart from '../../assets/heart.png';
import Calories from '../../assets/calories.png'; 
import CountUp from 'react-countup'
import {motion} from 'framer-motion';


const Hero = () => {

    const transition = {type: 'spring', duration: 3}
    const mobile = window.innerWidth<=768 ? true: false

  return (
    <div className='hero' id='home'>

        <div className="blur hero-blur"></div>
        <div className="left-h">
            <Header />
            {/* inicio the best ad */}
            <div className="the-best-ad">
                <motion.div
                initial={{left: mobile? "165px": "238px"}}
                whileInView={{left: '8px'}}
                transition={{...transition, type: 'tween'}}
                >

                </motion.div>
                <span>El mejor gimnasio de la ciudad</span>
            </div>
            {/* fin the best ad */}
            {/* inicio heading */}
            <div className="hero-text">
                <div>
                    <span className='stroke-text'>Forme</span>
                    <span> su</span>
                </div>
                <div>
                    <span>Cuerpo Ideal</span>
                </div>
                <div>
                    <span>aquí te ayudaremos a dar forma y construir tu cuerpo ideal y vivir tu vida al máximo</span>
                </div>
            </div>
            {/* fin heading */}
            {/* inicio figures */}
            <div className="figures">
                <div>
                    <span>
                        <CountUp start={100} end={140} prefix="+"  />
                    </span>
                    <span>coach expertos</span>
                </div>
                <div>
                    <span><CountUp start={800} end={978} prefix="+"  /></span>
                    <span>Acceso a miembros</span>
                </div>
                <div>
                    <span><CountUp start={0} end={50} prefix="+"  /></span>
                    <span>Programas de fitness</span>
                </div>
            </div>
            {/* fin figures */}
            {/* inicio botons */}
            <div className="hero-buttons">
                <button className="btn">Inicie ahora</button>
                <button className="btn">saber más</button>
            </div>
            {/* fin botons */}
        </div>
        <div className="right-h">
            <button className='btn'>Únete ahora</button>

            <motion.div
            initial={{right: '-1rem'}}
            whileInView={{right: '4rem'}}
            transition={transition}
            className="heart-rate">
                <img src={Heart} alt="" />
                <span>Heart Rate</span>
                <span>116 bpm</span>
            </motion.div>
            {/* inicio hero images */}
            <img className='hero-image' src={hero_image} alt="" />
            <motion.img
            initial={{right: '11rem'}}
            whileInView={{right: '20rem'}}
            transition={transition}
            className='hero-image-back' src={hero_image_back} alt="" />
            {/* fin hero images */}
            {/* inicio calories */}
            <motion.div
            initial={{right: '37rem'}}
            whileInView={{right: '28rem'}}
            transition={transition}
            className="calories">
                <img src={Calories} alt="" />
                <div>
                    <span>Calories Burned</span>
                    <span>220 kcal</span>
                </div>
                
            </motion.div>
            {/* fin calories */}
        </div>
    </div>
  )
}

export default Hero